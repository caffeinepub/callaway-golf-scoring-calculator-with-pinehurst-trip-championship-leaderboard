...
interface ExportLeaderboardPdfOptions {
  eventTitle: string;
  courseName?: string;
...
/**
 * Exports the leaderboard standings to a PDF file using browser print functionality.
 * This creates a print-optimized view and triggers the browser's print dialog.
 */
export function exportLeaderboardToPdf({ eventTitle, courseName, results, coursePar }: ExportLeaderboardPdfOptions): void {
  ...
  const documentTitle = eventTitle.trim() || 'Tournament Leaderboard';
  ...
  const htmlContent = `
    ...
    <html>
      <head>
        <meta charset="utf-8">
        <title>${escapeHtml(documentTitle)} - Leaderboard</title>
        <style>
          @media print {
            @page {
              margin: 0.75in;
      ...
      <body>
        <div class="header">
          ...
          <div class="subtitle">Tournament Leaderboard - Callaway Scoring System</div>
        ...
        <div class="footer">
          <p>Callaway Scoring System - Tournament Leaderboard</p>
